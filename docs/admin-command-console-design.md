# Admin Command Console — Design Document (ARCHIVED)

> **This is the archived design + as-built history.** For how the console works and how
> to add commands to it, see **[admin-command-console.md](admin-command-console.md)** —
> the living feature documentation. This file is kept for the full decision record and
> the step-by-step build history; it is **not** maintained going forward.

**Status:** **Implemented — Steps 0–6 complete (2026-06-26).** Design + build record.
**Author:** Josh + Claude (design + build sessions 2026-06-26)
**Implementation:** All six build steps landed on `main`: Step 0 (command normalization, §16),
Step 1 (log-buffer refactor, §8 "As-built"), Step 2 (backend skeleton — registry,
introspection, `CommandRun`, runner task, superuser routes; §13 "As-built"), Steps 3–4
(frontend: nav tab, console page, data-driven form renderer, command-line builder,
Execute/Open-in, polling log pane, run history + re-run, in-code docs in the detail pane;
§13 "As-built (Steps 3 & 4)"), and Steps 5–6 (safety polish — concurrency soft-block +
graceful broker-down dispatch — and this doc conversion; §13 "As-built (Steps 5 & 6)").

---

## 0. About this document

This file is **both** the original design spec **and** the living documentation for the
feature. The Admin Command Console is now built, so this document has been converted in
place — section by section — from *"here's what we'll build"* into *"here's how it
works."* That conversion (the Step 6 deliverable) is complete:

- The header **Status** is `Implemented` with the landing build steps.
- Forward-looking phrasing ("we will", "proposed", "should") now reads in the present
  tense describing the shipped behavior.
- The §13 build order is kept as the as-built record (one "As-built" note per step).
- The §14 questions are resolved and folded into the relevant sections.

**Ongoing maintenance:** keep the file/line references and the §12 enumeration current as
commands change — add a `CommandSpec` entry (§4b) for each new command and update §12.
...
| `enum:<name>` | **registry override only** | a named server-side list, single-select (e.g. `whisper_models` / `whisper_languages` for `backfill_transcripts`) |
| `enum_multi:<name>` | **registry override only** | a named server-side list, multi-select (e.g. `audio_origins` from `Episode.audio_origin()`) |
...
only names the handful argparse can't describe (`--origins`, the CSV positionals, and
the Whisper `--model`/`--language` enums).
...
- **Dispatch:** the `run` view validates the command is registered + `runnable`,
  ...
  confirmation, soft-blocks an identical in-flight run (409, §14), mints a `run_id`
  (UUID), **creates the `CommandRun` history row (§8a) in `queued` state**, kicks
  `task_run_management_command.delay()`, and returns the `run_id`.
- **Why Celery (decided):** matches `ingest_feed` / `gdrive_recovery`, survives the
  request lifecycle, doesn't tie up a gunicorn worker, and behaves behind Traefik. The
  console degrades gracefully when the worker is down: a missing-Celery import returns
  503 ("Celery/tasks unavailable…"), and if the broker itself is unreachable the
  `.delay()` failure is caught, the just-created `CommandRun` row is deleted (no orphan
  `queued` record), and the operator gets a 503 rather than a 500.
...
views. Routes (under `/admin-console/` to avoid colliding with Django's `/admin/`):
...
- **Danger flag** — destructive commands (`purge_r2_dev`, `purge_r2_media_dev`,
  ...
  and a typed confirmation before the Execute button enables — but only for a real
  `--apply`; a preview/dry run mutates nothing and dispatches straight away (both the JS
  `dangerActiveNow` and the `run` view enforce this). The underlying command's own
  `--apply`/`--yes` guards remain the real safety net.
...
- **Audit trail** — every dispatch is logged (who/what/redacted args) at INFO via the
  `pod_manager` logger, so it lands in the existing `LogEntry` viewer; the full
  `CommandRun` row is the durable record. An identical command already `queued`/`running`
  is **soft-blocked** (HTTP 409, §14 resolved) to stop accidental double-runs.
...
| `backfill_transcripts` | `--podcast` → podcast **multi-select**; `--stagger` → number (30); `--apply` → checkbox; `--model` → **enum picker** (`whisper_models`); `--language` → **enum picker** (`whisper_languages`); `--initial-prompt` → text; `--min/num/max-speakers` → number | ✅ | |
...
and `backfill_transcripts --model`/`--language` (single-select Whisper enums).

---

## 13. Build order (as-built)

All six steps shipped on 2026-06-26. Each carries an "As-built" note (in §16 for Step 0,
§8 for Step 1, and below for Steps 2–6) recording what actually landed and any deviations.
...
5. ✅ **Safety polish — DONE (2026-06-26).** Danger typed-confirm gate (front + back),
   audit logging of every dispatch via `CommandRun` + INFO → the `LogEntry` viewer,
   "discovered but unregistered" notice + the enforced §4c CI test, secret redaction,
   the §14 resolutions (concurrency soft-block; verbosity hidden; `result_summary`
   deferred), and graceful broker-down dispatch. As-built record below.
6. ✅ **Living documentation — DONE (2026-06-26).** This conversion (§0): Status flipped
   to Implemented, forward-looking phrasing moved to present tense, the build order kept
   as the as-built record, and the §14 questions folded into their sections.

---

### As-built (Step 2 — implemented 2026-06-26)

The backend skeleton landed; the frontend (Steps 3–4) consumes the JSON these routes
return. Ground truth:
...
(`feat: Admin Command Console frontend (Steps 3 & 4)`).

---

### As-built (Steps 5 & 6 — implemented 2026-06-26)

Most of the §11 safety surface had already landed inside Steps 2–4 (the danger
typed-confirm gate front and back, per-dispatch audit logging, secret redaction, the
unregistered notice and its enforced CI test, graceful "Celery import unavailable"
messaging). Step 5 closed the two genuine gaps and resolved the §14 questions; Step 6 is
this document.

**Code changes (Step 5)** — both in [views/admin_console.py](../pod_manager/views/admin_console.py) `run`:

1. **Concurrency soft-block (§14 resolved).** Before creating the history row, `run`
   rejects (HTTP **409**) a second *identical* invocation — same command **and** redacted
   `command_line` — while an earlier one is still `queued` or `running`. Two genuinely
   different invocations of the same command (e.g. two podcasts) don't collide; only an
   accidental repeat / double-click does. The check is one indexed `CommandRun` query.
2. **Graceful broker-down dispatch (§7).** The pre-existing guard only caught a missing
   Celery *import*. If the broker itself is down, `.delay()` raises (e.g. kombu
   `OperationalError`) — `run` now catches that, **deletes the just-created `CommandRun`
   row** so no orphan `queued` record lingers, logs a warning, and returns a clean **503**
   ("Celery broker unavailable…") instead of a 500.

**§14 questions — resolved:**

- **Verbosity / globals → hidden.** `--verbosity` and the argparse/Django built-ins stay
  in `_HIDDEN_DESTS` (schema.py); no "Advanced" disclosure was added (these commands
  write operator output to `self.stdout`, not via `--verbosity`). Covered by
  `AdminConsoleSchemaTests.test_globals_are_hidden_from_the_form`.
- **Concurrency → soft-block** (implemented above).
- **`result_summary` → deferred (v1 = log-only).** Every run still captures the full
  `log`; `result_summary` stays `null` until a command earns structured parsing. The
  field, model column, and JSON are already in place, so adding a parser later is additive.

**Already-done Step 5 items (verified, not rebuilt):** danger confirmation
([`run` gate](../pod_manager/views/admin_console.py) + JS `dangerActiveNow`); audit log
(the `run` INFO line routes through the `pod_manager` logger → `DatabaseLogHandler` →
`LogEntry`); the §4c notice + enforced test
(`AdminConsoleSchemaTests.test_every_discovered_command_is_registered` asserts
`unregistered_commands() == []`, so a new unregistered command fails CI); secret redaction
(`reconstruct_invocation` → redacted `args`/`options`/`command_line` in `CommandRun` and
the copy box).

**Tests.** Two new `AdminConsoleViewTests`: `test_run_soft_blocks_identical_in_flight_run`
(409 on the duplicate; a different invocation still dispatches) and
`test_run_503_and_drops_row_when_broker_down` (503 + no orphan row when `.delay()` raises).
Verified: `AdminConsoleViewTests` + `AdminConsoleSchemaTests` + `TaskRunManagementCommandTests`
green (**42 tests**); `manage.py check` clean.

---

### Follow-up — `backfill_transcripts` enum pickers (2026-06-27)

The §5a "optional Whisper enums" landed: `backfill_transcripts --model` / `--language`
(free-form `type=str` args) now get curated **single-select** pickers instead of bare
text boxes. New widget id **`enum:<name>`** (the single-valued sibling of
`enum_multi:<name>`): the registry override resolves to a named server-side list, the
schema ships its options inline, and the frontend renders a `<select>`. Two named lists
in [schema.py](../pod_manager/admin_console/schema.py) — `whisper_models` (standard sizes
+ `.en` variants + the `large` revisions) and `whisper_languages` (the common podcast
subset). Both stay optional (blank = use the network/podcast setting); an exotic
model/language not in the list is still reachable via the copy-box CLI, since the
underlying arg remains free-form. Covered by
`AdminConsoleSchemaTests.test_registry_override_single_enum`.

---

## 14. Decisions (all resolved)

Every question is now settled; nothing here is open. The originally-locked decisions:
...
The three questions that were open through Step 4, resolved in Step 5 (full rationale in
the §13 "As-built (Steps 5 & 6)" note):

- **Globals → hidden, no "Advanced" section.** `--verbosity` and the argparse/Django
  built-ins stay denylisted in `_HIDDEN_DESTS`; these commands surface progress via
  `self.stdout`, not `--verbosity`, so an Advanced disclosure earns nothing today (§5).
- **Concurrency → soft-block duplicates.** A second *identical* run (same command +
  redacted command line) while one is `queued`/`running` is rejected with HTTP 409; the
  `run` view enforces it (§7/§11).
- **`result_summary` → deferred (v1 = log-only).** The full `log` is the universal record
  for every run; `result_summary` stays `null` until a specific command earns structured
  parsing, which is purely additive (§8a).
