...
- **Cache-bust static JS that swapped pages load** (currently `creator_tabs.js?v=6` in `creator_settings.html`) whenever its execution contract changes. `STORAGES["staticfiles"]` is whitenoise's `CompressedStaticFilesStorage`, **not** the `Manifest` variant, so filenames are never hashed and this query param is the only explicit cache-bust in the project.

  Scale of the problem, so nobody over- or under-reacts: whitenoise serves unhashed files with `Cache-Control: max-age=60` (its default; `WHITENOISE_MAX_AGE` is not set), so a missed bump means roughly a **60-second** window where a returning browser runs the old file against new templates — not indefinite staleness. Within that window a stale copy executing inside a swap produces errors the current source can't explain, which is disproportionately confusing to debug for a one-line fix. Bump it. (If a CDN ever fronts `/static/` with a Browser Cache TTL that overrides origin headers, the window grows and the bump stops being optional — worth re-checking then.)
