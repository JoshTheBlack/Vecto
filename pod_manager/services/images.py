...
def _normalize_frame(img, max_px: int):
    """Centre-crop to square, bound to max_px, normalize mode for WebP."""
    w, h = img.size
    if w != h:
        s = min(w, h)
    ...
    with Image.open(field) as img:
        if getattr(img, 'is_animated', False):
            ...
            for frame in ImageSequence.Iterator(img):
                ...
                frames.append(_normalize_frame(frame.convert('RGBA'), max_px))
            buf = BytesIO()
            frames[0].save(
        ...
        img = _normalize_frame(img, max_px)
        buf = BytesIO()
