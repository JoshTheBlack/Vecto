...
def _download_to_temp(url: str) -> tuple[Path, str]:
    ...
    SSRF-guarded with the same validator the chapter fetch uses. The audio-vs-
    non-audio check happens once at the mirror chokepoint (mirror_episode_audio),
    so both this download and a caller-supplied local_path are guarded the same
    way. Caller owns the returned temp file and must unlink it.
    """
    ...
    # GDrive serves large files behind a "couldn't scan for viruses" HTML page
    # (HTTP 200); click through to the confirmed download so the real bytes stream.
    if gdrive_download.is_gdrive_url(url) and gdrive_download.looks_like_interstitial(resp):
        resp = gdrive_download.follow_confirmation(session, url, resp, 300)
        resp.raise_for_status()
...
def mirror_episode_audio(episode_id: int, local_path=None, force: bool = False) -> dict:
    ...
    try:
        ...
        # Single audio chokepoint — a download AND a caller-supplied local_path
        # (transcription's temp MP3) both converge here, so neither can ever
        # upload a non-audio body (an HTTP-200 HTML error page: GDrive quota /
        # "couldn't scan for viruses", a Patreon login wall). Positive magic-byte
        # detection; raises MirrorSkipped so a later re-run can still succeed.
        if not is_audio_file(upload_path):
            raise MirrorSkipped(f"episode {episode_id} source is not recognized audio "
                                f"(no audio signature in header)")
