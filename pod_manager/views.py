...
def generate_mix_feed(request, network_slug, unique_id):
    ...
    # FIX: Changed from '-published_date' to '-pub_date'
    ...
    for ep in episodes:
        ...
        feed.episodes.append(PodgenEpisode(
            title=display_title,
            # FIX: Using your exact field names from the error message
            summary=ep.clean_description or ep.raw_description, 
            publication_date=ep.pub_date,
            media=Media(
                url=ep.audio_url_subscriber, 
                size=0, # Defaulting to 0 since size isn't tracked in the model
                type="audio/mpeg"
            )
        ))
        
    # 6. Return the raw XML response
