...
def generate_mix_feed(request, network_slug, unique_id):
    """
    Generates the actual RSS XML file for a Custom Mix.
    Accessed directly by Apple Podcasts, Spotify, PocketCasts, etc.
    """
    cache_key = f"mix_feed_{unique_id}"
    feed_xml = cache.get(cache_key)

    if not feed_xml:
        # 1. Look up the mix by its unguessable UUID
        user_mix = get_object_or_404(UserMix, unique_id=unique_id, network__slug=network_slug, is_active=True)
        
        # 2. Security Check: Get the user's active pledge profile
        profile = user_mix.user.patron_profile
        user_cents = profile.pledge_amount_cents
        
        # 3. Filter the selected podcasts against the user's active authorization
        authorized_podcasts = []
        for podcast in user_mix.selected_podcasts.select_related('required_tier').all():
            req_cents = podcast.required_tier.minimum_cents if podcast.required_tier else 0
            if user_cents >= req_cents:
                authorized_podcasts.append(podcast)
                
        # 4. Fetch the episodes for ONLY the authorized shows
        episodes = Episode.objects.filter(
            podcast__in=authorized_podcasts
        ).select_related('podcast').order_by('-pub_date')[:100]
        
        # 5. Build the XML Feed using Podgen
        if user_mix.display_image:
            image_url = request.build_absolute_uri(user_mix.display_image)
        else:
            image_url = request.build_absolute_uri(user_mix.network.default_image_url)

        feed = PodgenPodcast(
            name=user_mix.name,
            description=f"A custom blended podcast feed generated by Vecto for {user_mix.name}.",
        ...
        for ep in episodes:
            ...
            feed.episodes.append(PodgenEpisode(
                title=display_title,
                summary=ep.clean_description or ep.raw_description, 
...
def creator_settings(request):
    ...
    """
    # 1. Determine which networks this user is allowed to manage
    ...
    if request.method == 'POST':
        ...
        if action == 'update_network':
            ...
            # Verify they own the network they are trying to update
        ...
        # --- ACTION: Update a specific Show's Settings ---
            ...
            # Ensure the show belongs to a network they own
...
def home(request):
    ...
    # Determine distinct networks for the filter dropdown
    ...
    # Contextual network theme loading
    current_network = None
    if selected_network:
        current_network = Network.objects.filter(slug=selected_network).first()
